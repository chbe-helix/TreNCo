...
then
awk '$3=="gene"' $annotation | awk -v fldno=12 -f $EXTRACT5p > $annotbase\_exons_most5p.gff
else
awk -v fileRef=$trbiotypes 'BEGIN{while(getline < fileRef >0){biotype["\""$1"\"\;"]=1;}} $3=="gene"{i=9; while($i!="transcript_type"){i+=2}if(($i=="transcript_type")&&(biotype[$(i+1)]==1)){print}}' $annotation | awk -v fldno=12 -f $EXTRACT5p > $annotbase\_exons_most5p.gff
fi
...
awk '{i=9; while($i!="transcript_type"){i++}if($i=="transcript_type"){split($(i+1),c,"\""); trbiot=c[2];} ($0~/cds_start_NF/) ? confidence="low" : confidence="not_low"; ($7=="+") ? tsspos=$4 : tsspos=$5; split($10,a,"\""); split($12,b,"\""); print $1, "Gencode", "CapSite", tsspos, tsspos, b[2], $7, ".", "gene_id", a[2], "tr", b[2], "trbiot", trbiot, "confidence", confidence;}' $annotbase\_exons_most5p.gff | awk -f $GFF2GFF > $annotbase\_capped_sites.gff
